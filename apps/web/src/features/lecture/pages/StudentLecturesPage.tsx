...
import { useStudentLectures } from "../hooks/useLectures";
...
export const StudentLecturesPage: React.FC = () => {
  ...
  const { data: lecturesData, isLoading, refetch } = useStudentLectures({
    page: currentPage,
    limit: pageSize,
  ...
  const metricsCards: TableMetricCard[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        val: allLectures.filter((l) => l.attendedLive || l.isCompleted).length,
        icon: CheckCircle2,
  ...
  const tabOptions: TableTabOption[] = [
    { key: "all", label: "All Classes", count: totalCount },
    { key: "LIVE_NOW", label: "Live Now", count: allLectures.filter((l) => l.liveStatus === "LIVE_NOW" || l.isLiveNow).length },
    { key: "SCHEDULED", label: "Upcoming", count: allLectures.filter((l) => l.liveStatus === "SCHEDULED").length },
  ...
  const sortOptions: TableSortOption[] = [
    ...
    { label: "Class Name (A - Z)", value: "title-asc" },
  ];

  const columns: TableColumn<Lecture>[] = [
    {
      header: "Live Class & Course",
      accessorKey: "title",
      cell: (row) => (
        <div className="space-y-1 max-w-xs sm:max-w-sm">
          <div
            ...
            onClick={() => navigate(`/students/lectures/${row.id}`)}
          >
            {row.title}
          </div>
          <div className="flex items-center gap-1.5 text-[11px] text-neutral-500">
            <BookOpen className="w-3 h-3 text-neutral-400 shrink-0" />
            <span className="truncate">{row.courseTitle || "Enrolled Course"}</span>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: (row) => {
        if (!row.scheduledAt) return <span className="text-xs text-neutral-400">TBD</span>;
  ...
  return (
    <div className="space-y-6">
      <DataTableTemplate<Lecture>
        ...
        title="My Live Lectures & Classes"
        description="View upcoming live sessions, schedule, and review agendas across your enrolled courses."
        headerActions={
          <Button
            variant="outline"
            size="sm"
        ...
        keyExtractor={(l) => l.id}
        isLoading={isLoading}
        searchPlaceholder="Search classes by name, course, or instructor..."
