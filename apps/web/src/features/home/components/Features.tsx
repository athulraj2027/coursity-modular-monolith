...
import { Check, Video, Mic, Layers, Zap } from "lucide-react"
        ...
        <div className="flex flex-col items-center text-center mb-20 space-y-4">
          ...
          <div className="max-w-4xl space-y-4">
            ...
            <p className="text-neutral-600 dark:text-neutral-400 text-base sm:text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
              Accelerate your engineering journey with live mentor sessions, automated real-time AI voice interview simulations, and comprehensive video curriculums.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {FEATURES_DATA.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="group relative flex flex-col justify-between rounded-2xl border border-neutral-200/80 dark:border-neutral-800/80 bg-white dark:bg-neutral-950/60 p-8 sm:p-10 transition-all duration-300 hover:border-[#F42A18]/60 hover:shadow-xl hover:shadow-[#F42A18]/5 text-left"
              >
                <div>
                  {/* Icon & Badge Header */}
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#F42A18]/10 text-[#F42A18] group-hover:bg-[#F42A18] group-hover:text-white transition-colors duration-200">
                      <IconComponent className="h-6 w-6" />
                    </div>
                    <span className="px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase bg-neutral-100 dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-800">
                      {item.badge}
                    </span>
                  </div>
